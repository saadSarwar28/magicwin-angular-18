...
import { NavbartopComponent } from './standalone/navbartop/navbartop.component';
import { ReCaptchaV3Service } from 'ng-recaptcha';
import { RECAPTCHA_V3_SITE_KEY } from 'ng-recaptcha';
import { NavBottomComponent } from './standalone/nav-bottom/nav-bottom.component';
import { SitefooterComponent } from './standalone/sitefooter/sitefooter.component';
...
@Component({
  ...
  providers: [
    ReCaptchaV3Service,
    {
      ...
      useValue: '6LfaGQQeAAAAAHGQ_EEv9PWEu8pQE_suL2WUSL7h',
...
export class AppComponent implements OnInit {
  ...
  title =
    this.browserService.getWindow() && this.browserService.getWindow().sitename;
    ...
    if (this.platformService.isBrowser()) {
      ...
      if (this.browserService.getWindow().isIframe) {
        this.isIframe =
          ...
          this.browserService.getWindow().isIframe;
      }
      if (this.browserService.getWindow().cdnUrl) {
        this.cdnUrl =
          this.browserService.getWindow() &&
          this.browserService.getWindow().cdnUrl;
      }
      if (this.browserService.getWindow().showWhatsAppIconInBottom) {
        this.showWhatsAppIconWithText =
          this.browserService.getWindow() &&
          this.browserService.getWindow().showWhatsAppIconInBottom;
      }
    }
  }
  ...
  updateOnlineStatus(event: Event) {
    if (this.platformService.isBrowser()) {
      this.isOffline = !this.browserService.getNavigator().onLine;
      console.log(
        `Online status changed: ${this.isOffline ? 'Offline' : 'Online'}`
      );
    }
  }
  ...
  ngOnInit(): void {
    if (this.isBrowser) {
      ...
      this.isMobile =
        this.browserService.getWindow() &&
        this.browserService.getWindow().innerWidth <= 992;
      ...
      if (
        this.browserService.getNavigator() &&
        this.browserService.getNavigator().webdriver
      ) {
  ...
  addGoogleTag() {
    const dynamicId =
      this.browserService.getWindow() &&
      this.browserService.getWindow().gtmCode; // Ensure this value is set globally
    ...
    this.renderer.setAttribute(
      iframe,
      'src',
      `https://www.googletagmanager.com/ns.html?id=${dynamicId}`
    );
  ...
  setFavicon() {
    ...
    const imgSrc =
      this.browserService.getWindow() &&
      this.browserService.getWindow().faviconSrc; // Assuming window.faviconSrc is set
  ...
  setIndexHeadMetaData() {
    ...
    const metaContentName =
      this.browserService.getWindow() &&
      this.browserService.getWindow().keywordName;
    const metaContent =
      this.browserService.getWindow() &&
      this.browserService.getWindow().keywordContent;
    // Add new meta tag dynamically
    this.metaService.addTag({
      name: metaContentName,
      content: metaContent,
    });
    const metaContentName2 =
      this.browserService.getWindow() &&
      this.browserService.getWindow().keywordName2;
    const metaContent2 =
      this.browserService.getWindow() &&
      this.browserService.getWindow().keywordContent2;
    // Add new meta tag dynamically
    this.metaService.addTag({
      name: metaContentName2,
      content: metaContent2,
    });
    const metaContentName3 =
      this.browserService.getWindow() &&
      this.browserService.getWindow().keywordName3;
    const metaContent3 =
      this.browserService.getWindow() &&
      this.browserService.getWindow().keywordContent3;
    // Add new meta tag dynamically
    this.metaService.addTag({
    ...
    this.metaService.addTag({
      ...
      content: `${
        this.browserService.getWindow() &&
        this.browserService.getWindow().sitename
      } : Best Online Sports Betting Site in India 2024`,
    });
    this.metaService.addTag({
      ...
      content: `${
        this.browserService.getWindow() &&
        this.browserService.getWindow().sitename
      } : India's top legal betting site. Enjoy fast withdrawals, top-notch betting, and 5000+ casino games. Get a 20% welcome bonus up to ₹10,000. Sign up now!`,
    });
    ...
    this.metaService.addTag({
      ...
      content:
        this.browserService.getWindow() &&
        this.browserService.getWindow().siteLogo,
    });
    this.metaService.addTag({
      name: 'twitter:image',
      content: `@${
    ...
    const metaProperty =
      this.browserService.getWindow() &&
      this.browserService.getWindow().property;
    const metaPropertyContent =
      this.browserService.getWindow() &&
      this.browserService.getWindow().propertyContent;
    ...
    const metaProperty2 =
      this.browserService.getWindow() &&
      this.browserService.getWindow().property2;
    const metaPropertyContent2 =
      this.browserService.getWindow() &&
      this.browserService.getWindow().propertyContent2;
    // Create the meta tag with property and content attributes
  ...
  showDownloadAppModal() {
    if (
      this.isDownloadAppPopupCookieExpired() &&
      !this.isApk() &&
      !this.isIphone()
    ) {
      setTimeout(() => {
