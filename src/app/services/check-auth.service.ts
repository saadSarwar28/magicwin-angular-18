...
export class CheckAuthService {
  ...
  HaveStakes(): boolean {
    const stakes = this.storageService.getItem('stakes');
    if (stakes) {
  ...
  getstaks() {
    let b = this.storageService.getItem('stakes');
    if (b) {
      return JSON.parse(b);
    } else {
      return {
  ...
  ClientParmeters() {
    this.sportsService
      ...
      .subscribe((r: ClientParameters) => {
        if (this.storageService.getItem('stakes') == null) {
          this.storageService.setItem('stakes', JSON.stringify(r));
        }
        this.cBuyRate = r.cBuyRate;
        this.currencyCode = r.currencyCode + '-';
        this.cTotalShare = r.cShare + r.pShare;
